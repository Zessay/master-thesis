...
class HredModel(object):
	def __init__(self, data, args, embed):
		...
		with tf.variable_scope('encoder'):
			...
			encoder_output, encoder_state = tf.nn.dynamic_rnn(cell_enc, self.encoder_input, self.posts_length, dtype=tf.float32,
														scope="encoder_rnn")
		...
		with tf.variable_scope('context'):
			...
			context_output, self.context_state = tf.nn.dynamic_rnn(cell_ctx, encoder_state_reshape, self.context_length,
